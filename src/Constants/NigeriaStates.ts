/**
 * Nigeria — all 36 states + the Federal Capital Territory (Abuja), each mapped
 * to its complete list of Local Government Areas (LGAs). 774 LGAs total.
 *
 * Used to drive the dependent "State Of Origin" → "Local Government" dropdowns
 * in the Add Student form: pick a state, and its LGAs populate the second
 * dropdown.
 *
 * States are keyed alphabetically; LGA arrays are alphabetical within each
 * state. Source: official INEC / NBS local government listings.
 */
...
/** All state names, alphabetical — for the "State Of Origin" dropdown. */
export const NIGERIAN_STATES: string[] = Object.keys(STATE_LGAS);

/** LGAs for a given state, or [] if the state is unknown/unset. */
export function getLGAsForState(state: string | null | undefined): string[] {
  if (!state) return [];
