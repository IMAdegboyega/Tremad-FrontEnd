...
import Image from 'next/image';
  ...
  /**
   * Basic student information for preview view
   */
interface Student {
    fullName: string;
    studentId: string;
  ...
  /**
   * Complete student information for full view table
   */
...
interface StudentActivity {
  ...
  /**
   * Props for the StudentLogin component
   */
interface StudentLoginProps {
    ...
    //new prop
    onQuickAction?: (studentId: string, action: 'reset' | 'deactivate' | 'delete') => void;
  }

  /**
   * StudentLogin component with three view modes:
   * 1. Preview view - compact table
   * 2. Full view - complete table with search/filter
   * 3. Detail view - individual student profile and history
   */
const StudentLogin: React.FC<StudentLoginProps> = ({ 
    ...
    // ✅ ADD THIS - Render full history view
    if (isHistoryView && selectedStudent) {
      return (
        <div className="min-h-screen bg-gray-50 p-8">
          ...
          <div className="mb-6">
            ...
            <h1 className="text-2xl font-semibold text-gray-900">
              Recent History - {selectedStudent.fullName}
            </h1>
          </div>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      {selectedStudent.studentId} • {selectedStudent.grade} - {selectedStudent.course} • Admission # {selectedStudent.admissionNo}
                    </p>
                  </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-8">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-semibold text-gray-900">Student login details</h1>
        </div>    
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  <th className="text-left text-xs font-medium text-gray-500 px-6 py-3">Students</th>
                  <th className="text-left text-xs font-medium text-gray-500 px-6 py-3">Admission no</th>
              ...
              <tbody>
                {/* Render each student row with status indicators */}
                    ...
                    <td className="px-6 py-4">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => onQuickAction?.(student.id, 'reset')}><span className='cursor-pointer'>Reset Password</span></DropdownMenuItem>
                          <DropdownMenuItem onClick={() => onQuickAction?.(student.id, 'deactivate')}><span className='cursor-pointer'>Deactivate User</span></DropdownMenuItem>
                          <DropdownMenuItem onClick={() => onQuickAction?.(student.id, 'delete')}>
    ...
    <div className="bg-white rounded-lg border border-gray-100 p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-lg font-semibold text-gray-900">
          Student Logins <span className="text-gray-500 font-normal">(980 accounts)</span>
        </h2>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gray-50 items-center justify-center p-2 border-b border-gray-100">
              ...
              <th className="text-left text-xs font-medium text-gray-500 p-2">Student ID</th>
              <th className="text-left text-xs font-medium text-gray-500 p-2">Username</th>
            </tr>
          </thead>
          <tbody>
            {/* Render basic student information rows */}
            {students.map((student, index) => (
              <tr key={index} className="border-t border-gray-100 last:border-b-0">
                <td className="p-2 pt-4 pb-4 text-sm text-gray-900">{student.fullName}</td>
                <td className="p-2 pt-4 pb-4 text-sm text-gray-600">{student.studentId}</td>
                <td className="p-2 pt-4 pb-4 text-sm text-gray-600">{student.username}</td>
              </tr>
            ))}
          </tbody>
        </table>
